...
const t = {
  kr: {
    ...
    titleMain: '매운맛',
    titleSub: '챌린지',
    desc: '당신의 한계를 시험하세요. 박스 안의 시크릿 코드를 인증하고, 당신의 지역에 CY를 더해 글로벌 랭킹을 올리세요.',
...
export default function Home({ lang }) {
  ...
  const handleCategoryChange = (e) => {
    ...
    setFormData({ ...formData, category: newCat, targetId: defaultTarget });
    setErrorMsg('');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.serial || formData.serial.length < 8) {
      setErrorMsg(text.errorInvalid); return;
    }
    ...
    try {
      const { data: codeData, error: codeError } = await supabase
        .from('serial_codes').select('*').eq('code', codeUpper).single();
      ...
      const { error: rankError } = await supabase
        .from('rankings').update({ score: newScore }).eq('id', formData.targetId);

      if (rankError) throw rankError;

      await fetchLeaderboard();
      triggerConfetti();
      setSuccessData({ points: POINTS_PER_CHALLENGE, targetName: targetName, serial: codeUpper });
        ...
        <section className="w-full max-w-md bg-spicy-black-light/40 backdrop-blur-xl border border-white/5 rounded-3xl p-8 md:p-10 shadow-2xl mb-24 flex flex-col relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 relative z-10">
            <div className="flex flex-col gap-4">
              <div>
                ...
                <div className="relative">
                  <select value={formData.category} onChange={handleCategoryChange} className="w-full bg-black/60 border border-white/10 rounded-xl px-4 py-3.5 text-white appearance-none focus:outline-none focus:border-spicy-red-light transition-colors cursor-pointer">
              ...
              <div>
                ...
                <div className="relative">
                  <select name="targetId" value={formData.targetId} onChange={handleInputChange} className="w-full bg-black/60 border border-white/10 rounded-xl px-4 py-3.5 text-white appearance-none focus:outline-none focus:border-spicy-red-light transition-colors cursor-pointer">
            ...
            <div className="mt-2">
              ...
              <input type="text" name="serial" value={formData.serial} onChange={handleInputChange} placeholder={text.placeholderCode} maxLength={12} className="w-full bg-black/80 border border-white/20 rounded-xl px-4 py-4 text-white text-lg font-mono tracking-[0.2em] focus:outline-none focus:border-spicy-red transition-all placeholder:text-gray-700 uppercase" />
            </div>
            {errorMsg && (
            ...
            <button type="submit" disabled={isSubmitting} className={`mt-2 w-full bg-gradient-to-r ${isSubmitting ? 'from-gray-700 to-gray-800 delay-0' : 'from-spicy-red to-spicy-red-dark hover:from-spicy-red-light hover:to-spicy-red shadow-[0_0_20px_rgba(230,0,0,0.3)] hover:shadow-[0_0_35px_rgba(230,0,0,0.5)]'} text-white font-bold py-4 px-6 rounded-xl transition-all transform hover:-translate-y-1 active:scale-[0.98] duration-300 flex justify-center items-center`}>
              {isSubmitting ? <div className="w-6 h-6 border-2 border-white/20 border-t-white rounded-full animate-spin"></div> : text.btnSubmit}
            </button>
          </form>
        </section>
        ...
        <section className="w-full">
          <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-10 gap-6 border-b border-white/10 pb-6">
            ...
            <div className="flex bg-black/50 p-1.5 rounded-2xl border border-white/5">
              {[{ id: 'korea', label: text.tabKorea }, { id: 'global', label: text.tabGlobal }].map((tab) => (
                <button
                  key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`px-6 py-2.5 rounded-xl text-sm font-bold tracking-wide transition-all duration-300 ${activeTab === tab.id ? 'bg-spicy-red shadow-[0_0_15px_rgba(230,0,0,0.4)] text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                >{tab.label}</button>
              ))}
            </div>
          </div>
          <div className="flex flex-col gap-4 relative min-h-[400px]">
            {leaderboard[activeTab].map((item, i) => {
              ...
              return (
                <div key={item.id} className="group relative bg-black/40 border border-white/5 rounded-2xl p-5 flex items-center justify-between overflow-hidden transition-all duration-300 hover:bg-white/5 hover:border-white/10 hover:-translate-y-0.5">
                  <div className="absolute top-0 left-0 h-full bg-gradient-to-r from-spicy-red/10 to-spicy-red/5 transition-all duration-1000 ease-out group-hover:from-spicy-red/20" style={{ width: `${progressPercent}%` }}></div>
                  <div className="relative z-10 flex items-center gap-6 w-full max-w-[60%]">
                    <div className="text-2xl font-black font-display w-8 text-right opacity-80">
                      {i === 0 ? <span className="text-[#FFD700] drop-shadow-[0_0_8px_rgba(255,215,0,0.5)]">1</span> : i === 1 ? <span className="text-[#C0C0C0]">2</span> : i === 2 ? <span className="text-[#CD7F32]">3</span> : <span className="text-gray-600">{i + 1}</span>}
                    </div>
      ...
      {successData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-black/60 animate-in fade-in duration-300">
          <div className="bg-spicy-black border border-spicy-red/30 rounded-3xl p-8 max-w-sm w-full text-center shadow-[0_0_50px_rgba(230,0,0,0.2)] animate-in zoom-in-95 duration-300">
            <div className="w-20 h-20 bg-spicy-red/20 rounded-full flex items-center justify-center mx-auto mb-6"><span className="text-4xl">🔥</span></div>
            ...
            <button onClick={() => setSuccessData(null)} className="w-full bg-white text-black font-bold py-4 rounded-xl hover:bg-gray-200 transition-colors">
              {text.modalBtn}
            </button>
          </div>
        </div>
